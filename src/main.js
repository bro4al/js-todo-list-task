...
class TodoService {
    ...
    changeTheme() {
        ...
        document.querySelectorAll('.todo-list__item').forEach(item => {
            item.classList.add(newTheme);
            item.classList.remove(oldTheme);
            item.children[0].children[0].children[0].classList.add(newTheme); 
            item.children[0].children[0].children[0].classList.remove(oldTheme); 
        });
...
class MainService {
    ...
    _onOpenModal() {
        if((+localStorage['currentPage']+1)*(+localStorage['cardsOnPage']) < 100) {
            ...
            alert("There are limit - 100 cards!");
        } else {
